...
class Index:
    ...
    def objects_to_indexes(self, object_seq: Sequence[Any]) -> np.ndarray:
        ...
        """
        self.not_in_index = start - 1
        return np.array(
            [self.word_to_index.get(obj, self.not_in_index) for obj in object_seq], dtype = int
        )
    ...
    def objects_to_index_matrix(
        ...
        matrix = np.full((len(object_seq_seq), max_len), self.not_in_index, dtype = int)

        for i, seq in enumerate(object_seq_seq):
            matrix[i, :len(seq)] = self.object_to_index(seq)
        return matrix
